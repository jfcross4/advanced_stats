...
# Example 2: Back to the Pizza Party

In class, we imagined that individual guests had the following chances of consuming different amount of pizza.
...
Using our knowledge of random variables we found that 50 guests would be expected to consume 60 slixes wiht a standard deviation of 6.93 slices and has a 25.8% chance of consuming more than 64 slices (8 pies).  Let's see if we can replicate this using bootstrapping!

First, we'll creating a vector of 50 guests and distribute their pizza consumption according to the table above:

```r
pizza_slices <- c(rep(0, 15), rep(1, 15), rep(2, 15), rep(3, 5))

pizza_slices
```

Next, let's make 100,000 bootstrapped samples of this vector:

```r
boot_samples = 
replicate(1e5, sample(pizza_slices, size=50,replace=TRUE))
```

Now, we'll find the total pizza consumption in each sample of 50 guests:

```r
tot_consumed <- apply(boot_samples, 2, sum)
```

We can find the mean and standard deviation of the amount of pizza consumed:
...
We can also find the chance that the guests consume more than 64 slices:

```r
mean(tot_consumed > 64)
```

How does this compare to what we found doing our random variable analysis?


# Example 3: Weighting a Kangaroo

Suppose that we're trying to weigh a kangaroo but it's jumping up and down.  Like good statisticians, we decide to weigh it a number of times and take the average.
...
# Example 4: A Cubit and Foot Length Correlation
...
# Example 5: Shooting Free Throws!
