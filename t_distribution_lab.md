...
Do you remember our kangaroo weights from a few weeks ago?  We imagined weighing the same jumping kangaroo ten times and averaging the noisy measurements to estimate its weight.
...
var(sample_means)
sqrt(var(sample_means))
```

There is another way to do this!  We can calculate the answer without using simulation.  We'll start by using the variance of our measured kangaroo weights to determine the inaccuracy of our measurements.  

**Which variance?!?!**  (you now know to ask)

If we're interested in how accurate are measurements are, we looking for the variance in the imaginary population of all possible measurements (or some very large number of measurements if you prefer).  

<!---(We only have a sample of that population.  To estimate the variance in the full population we should use the (Bessel corrected) variance of the same.  This the one that divides by $n-1$ instead of $n$. This is also the default of the "var" function in R.  This gives us an unbiased estimate (*but still just an estimate*) of the variance in my kangaroo measuring.)
-->
...
Since variances add, the variance is the **sum** of 10 measurements would be 10 times this large.  The **mean** of 10 measurements is 1/10th of the sum of 10 measurements but that actually implies that the variance in the mean of 10 measurements is 100 smaller than the variance in the sum of 10 measurements (from our law of linear transformations from earlier in the year):

$$var(aX) = a^2 var(X)$$

Therefore, the **variance in the mean** of 10 measurements in 1/10th the variance in the measurements.  More generally:
...
Stop here and ask yourself: What is the difference between the standard deviation in kangaroo weights and the standard error in kangaroo weights?  What questions are they answering?  When you and your neighbor have both encountered this question, share your answers with each other.

# Are Our Confidence Intervals Overconfident?
...
No, not quite!  It would be right if we knew for certain the variance in our measurements but we don't.  We have an estimate of the variance in our measurements.  There are two sources of uncertainty here, uncertainty due to the limited number of measurements (and the variance in the measurements we do have) and uncertainty in how inherently varied my kangaroo weight measurements really are.  This added source of variance is why statisticians use the *t* distribution rather than the standard normal distribution.  The t distribution accounts for the uncertainty in my estimate of the variance in the measurements.
...
I'm once again asking you to talk with your neighbor.  Try to explain why this confidence interval is wider than the one we created using a normal distribution.

## Problem # 1

You land on the planet Stannus and meet the Stanni who live there.  You find the masses of 6 Stanni in kilograms:
