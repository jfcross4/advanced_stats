---
title: "Chi Square Goodness of Fit Lab"
author: "Jared Cross"
date: "`r Sys.Date()`"
...
In an experiment, two different species of fruit trees were crossbred. The resulting fruit from this crossbreeding experiment were classified by color of meat of the fruit and color of the skin of the fruit, into one of four groups, as shown in the table below.
...
Next, it's time to calculate a test statistic!  We could (and will ultimately) choose $\chi^2$ as our test statistic, but, for fun, let's first try to use "George" which defined in class:
...
Is the "george" for this sample smaller or large than the "george" for the actual fruit trees?  (Note: everyone has a different simulation so your answer might differ from your neighbors.)
...
```
Not that many!  Only 2.46% (for me, your simulation might have turned out a bit differently!).  This is a p-value.  If the botanist is right, a result this far from their expectations (or further) should only happen 2.46% of the time.
...
```r
simulated_chi_square_values = replicate(10000, 
{sample_fruits = sample.int(4, 150, replace=TRUE, prob=c(5,2,2,1))
...
```
My simulation gives a p-value of 2.26%. Using the chi square test statistic this result seems slightly more surprising given the botanist theory than using the George test statistic... but it's not THAT different.  
...
chisq.test(x = observed, p=prop_expected)
```
...
Please analyze this data in light of the ornithologist's concerns.  Does this data provide strong evidence that grackles are an increasing the proportion of the bird population?  You may use a chi square test statistic or any test statistic that you think makes sense.